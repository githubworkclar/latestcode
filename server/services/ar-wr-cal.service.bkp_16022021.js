...
let ORingGlandAnalysisService = class ORingGlandAnalysisService {
        ...
        try {
            ...
            if (data.serviceStorageTemperatureAmbient75FMax > compositeMatlCheckDetail.serviceStorageTemperatureAmbientMax) {
                rule[i] = {
                    ...
                    'detail': 'Check Temperature Range'
                };
                i++;
            ...
            // let interfaceP: number = output.estimatedInterfacePressureAtHighestTemperature = eval(pressInDetails[0].interfaceP.formula);
            ...
            // let maxP: number = output.estimatedCollapsePressureatHighestTemperature = eval(pressInDetails[0].maxP.formula);
            ...
            this.logger.info(`estimatedCollapsePressureatHighestTemperature ${maxP}`);
    ...
    async getSumOfPressInACGrowthWR600(calType, unit, serviceStorageTemperatureAmbient75FMax, rotatingElementODMax, stationaryElementIDMax, composite) {
        ...
        try {
            ...
            // this.logger.info(`master r ${JSON.stringify(cteCalculationMasterQueryResult)}`);
        }
        catch (error) {
            // throw error so that promise automatically will be rejected
            // handle error
        ...
        ///// this.logger.info(`aeGrowth: ${aeGrowth}`);
        const sumPromise = new Promise((resolve, reject) => {
            // get from db
            let cteCalcsDataQuery;
            if (constants_1.Units.MILLI_METER === unit) { // Millimeter or Metric
                    ...
                    if (result.length > 0) {
                        ...
                        for (let index = 0; index < dbResult.compositeMaterials[0].temperatures.length; index++) {
                            ...
                            if (serviceStorageTemperatureAmbient75FMax > nextTemp) {
                                ...
                                // this.logger.info(`aeGrowth original if temp ${temperature} nextTemp ${nextTemp}: ${aeGrowthCalc}`);
                                // aeGrowth = aeGrowth + parseFloat((aeGrowthCalc ? aeGrowthCalc : 0.000).toFixed(5));
                                const aeGrowthCalc = eval(cteCalculationMasterQueryResult[0].aeGrowth1.formula);
                                aeGrowth = aeGrowthCalc ? aeGrowthCalc : 0.000;
                                this.logger.info(`aeGrowth if temp ${temperature} nextTemp ${nextTemp}: ${aeGrowth}`);
                                // aeGrowth
                                ...
                                aiGrowth = aiGrowthCalc ? aiGrowthCalc : 0.000;
                                this.logger.info(`aiGrowth if temp ${temperature} nextTemp ${nextTemp}: ${aiGrowth}`);
                            }
                            else {
                                // aeGrowth
                                const aeGrowthCalc = eval(cteCalculationMasterQueryResult[0].aeGrowth2.formula);
                                // aeGrowth = aeGrowth + parseFloat((aeGrowthCalc ? aeGrowthCalc : 0.000).toFixed(5));
                                aeGrowth = parseFloat((aeGrowthCalc ? aeGrowthCalc : 0.000).toFixed(5));
                                this.logger.info(`aeGrowth else temp ${temperature} nextTemp ${nextTemp}: ${aeGrowth}`);
                                // aiGrowth
                                const aiGrowthCalc = eval(cteCalculationMasterQueryResult[0].aiGrowth2.formula);
                                // aiGrowth = aiGrowth + (aiGrowthCalc ? aiGrowthCalc : 0.000);
                                aiGrowth = aiGrowthCalc ? aiGrowthCalc : 0.000;
                                this.logger.info(`aiGrowth else temp ${temperature} nextTemp ${nextTemp}: ${aiGrowth}`);
                            }
                            this.logger.info('acGrowth formula ' + cteCalculationMasterQueryResult[0].acGrowth1.formula);
                            const calculatedAcGrowth = eval(cteCalculationMasterQueryResult[0].acGrowth1.formula);
                            this.logger.info(`acGrowth temp ${temperature} nextTemp ${nextTemp}: ${calculatedAcGrowth}`);
                            acGrowth = acGrowth + (calculatedAcGrowth ? calculatedAcGrowth : 0.000);
                        }
                        // const acGrowth = eval(cteCalculationMasterQueryResult[0].acGrowth1.formula);
                        this.logger.info(`acGrowth: ${acGrowth}`);
...
//# sourceMappingURL=ar-wr-cal.service.bkp_16022021.js.map